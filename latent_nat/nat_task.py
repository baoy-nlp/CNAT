...
class NATGenerationTask(TranslationLevenshteinTask):
    ...
    def add_args(parser):
        ...
        parser.add_argument(
            ...
            default=False,
            action='store_true',
            help="set True if the NAT need generate a reference-length sentence"
        )
        parser.add_argument(
            '--infer-with-tgt',
            default=False,
            action='store_true',
            help="oracle analysis, whether the NAT model can access the target tokens"
        )
        parser.add_argument(
            "--no-accuracy",
            action='store_true',
            default=False,
            help="set no-accuracy if employs multiple GPU training"
        )
        parser.add_argument(
            '--ptrn-model-path',
            type=str,
            default=None,
            help="used for initialize the model"
        )
        parser.add_argument(
            "--extract-code",
            action='store_true',
            default=False,
            help="extra the learned latent codes for oracle analysis"
        )
        parser.add_argument(
            "--posterior-warmup-updates",
            type=int,
            default=0,
            help="to deprecate, warmup steps for training the Latent Posterior"
        )
        parser.add_argument(
            "--posterior-anneal-updates",
            type=int,
            default=0,
            help="to deprecate, training steps for training the Latent Posterior"
        )

    def build_generator(self, models, args, **unused):
        # add inference parameters to match the API for SequenceGenerator
    ...
    def build_model(self, args):
        # set the generator parameter for validation with BLEU score
        # extend from TranslationTask
        model = super(TranslationLevenshteinTask, self).build_model(args)
