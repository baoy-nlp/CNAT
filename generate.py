...
def main(args):
    ...
    assert not getattr(args, "glancing", False) or args.nbest == args.beam, \
...
def _main(args, output_file):
    ...
    if args.max_tokens is None and getattr(args, "max_sentences", None) is None:
    ...
    for sample in progress:
        ...
        gen_timer.start()
        if getattr(args, "extract_code", False):
            hypos = task.inference_step(generator, models, sample, prefix_tokens, extra_code=True)
        else:
                ...
                if not args.quiet:
                    ...
                    if "ref_code" in hypo:
                        print('RC-{}\t{}'.format(
                            sample_id,
                            ' '.join(map(
                                lambda x: '{}'.format(x), hypo['ref_code'].tolist(),
                            ))
                        ), file=output_file)

                    if "hypo_code" in hypo:
                        print('HC-{}\t{}'.format(
                            sample_id,
                            ' '.join(map(
                                lambda x: '{}'.format(x), hypo['hypo_code'].tolist(),
                            ))
                        ), file=output_file)
    ...
    if has_target:
        if args.bpe and not args.sacrebleu:
            if args.remove_bpe:
                logger.warning("BLEU score is being computed by splitting detokenized string on spaces, "
                               "this is probably not what you want. Use --sacrebleu for standard 13a BLEU tokenization")
            else:
